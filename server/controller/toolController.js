...
exports.insert = (req, res, callback) => {
  ...
  if (!req.file) {
    ...
    const sql =
      "INSERT INTO tool (tool_name, tool_description, tool_img) VALUES (?,?,?)";
  ...
  try {
    db.query(sql, [id], (err, results) => {
      ...
      if (req.file) {
        ...
        const imagePath = `/uploads/${req.file.filename}`;
        const sql =
          "UPDATE tool SET tool_name = ?, tool_description = ?,tool_img = ? WHERE id = ?";
        db.query(
          sql,
          [tool_name, tool_description, imagePath, id],
          (err, result) => {
            if (err) {
              throw err;
            }
        ...
        const sql =
          "UPDATE tool SET tool_name = ?, tool_description = ? WHERE id = ?";
